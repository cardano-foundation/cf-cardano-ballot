...
@Service
public class SimpleExpirationService implements ExpirationService {
    ...
    public boolean isEventActive(Event event) {
        ...
        return switch (event.getEventType()) {
            case PERSON_BASED -> (absoluteSlot >= event.getStartSlot() && absoluteSlot <= event.getEndSlot());
    ...
    public boolean isEventFinished(Event event) {
        ...
        return switch (event.getEventType()) {
            case PERSON_BASED -> (absoluteSlot > event.getEndSlot());
