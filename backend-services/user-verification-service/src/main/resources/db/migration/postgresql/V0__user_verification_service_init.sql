...
create index idx_sms_wallet_id_status on sms_user_verification(event_id, wallet_type, wallet_id, status);

create index idx_sms_status_phone_hash on sms_user_verification(event_id, status, phone_number_hash);

create index idx_sms_wallet_id_status_phone_hash on sms_user_verification(event_id, wallet_type, wallet_id, status, phone_number_hash);

create index idx_sms_wallet_id_status_req_id on sms_user_verification(event_id, wallet_type, wallet_id, status, request_id);
...
CREATE TABLE discord_user_verification (
   ...
   wallet_type VARCHAR(255), -- nullable since it is set on check-verification request
...
create index idx_discord_event_id on discord_user_verification(event_id);
