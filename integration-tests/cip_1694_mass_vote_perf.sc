...
val blockfrostApiKey = "preprod9v5e2dLyUa88oRBNQctiIX469J204eeO"

//val blockfrostUrl = "https://blockfrost-api.pro.dandelion-preprod.eu-west-1.metadata.dev.cf-deployments.org/api/v0/"
...
val eventName = "CIP-1694_Pre_Ratification_PER2"
...
""".stripMargin
    ...
    val r = requests.post(
        "https://api.perf.cf-cip1694-perf.eu-west-1.metadata.dev.cf-deployments.org/api/vote/cast",
        headers = Map(
...
def latestAbsoluteSlot(mapper: ObjectMapper): Long = {
    val r = requests.get(
        "https://follower-api.perf.cf-cip1694-perf.eu-west-1.metadata.dev.cf-deployments.org/api/blockchain/tip",
        headers = Map("Content-Type" -> "application/json")
    )
    ...
    if (r.statusCode == 200) {
        ...
        val slot = tree.get("absoluteSlot").asLong()
...
@main
def main(isAlreadyRegistered: Boolean = true) = {
...
//
//            Thread.sleep(60 * 1000L) // lets wait until voting-app and verification app ingest event and category data
//
//            println("woke up...")
