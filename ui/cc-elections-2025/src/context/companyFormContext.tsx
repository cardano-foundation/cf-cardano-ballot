...
export const CompanyFormContext = createContext<FormContextType<CompanyFormData> | null>(null);

type FormProviderProps = {
  children: ReactNode;
}

export const CompanyFormProvider = ({ children }: FormProviderProps) => {
  const title = [
  ...
  const req = [
    ...
    [],
    [],
    ['name', 'registrationNumber', 'keyContactPerson', 'email', 'country', 'publicContact'],
    [],
    [],
    [],
  ]
  ...
  return (
    <CompanyFormContext.Provider value={{title, page, setPage, data, setData, handleChange, error, setError, req}}>
      {children}
