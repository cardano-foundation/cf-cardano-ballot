...
type Errors = { errors: Array<{ errorCode: string }> } & Omit<
  Response,
  "errors"
>;
  ...
  return {
    async parse(response: Response | AnuthorizedResponse): Promise<T | never> {
      ...
      const contentType = response.headers.get(
        Headers.CONTENT_TYPE.toLowerCase(),
      );
      const isJson =
        contentType && contentType.indexOf(MediaTypes.APPLICATION_JSON) !== -1;
      try {
        ...
        if (response.status !== 200) {
          throw new HttpError(
            ...
            response.status === 500
              ...
              : getErrorMessage(parsedResponse),
          );
        }
      } catch (error: any) {
      ...
      if (
        ...
        parsedResponse.errors.length >= 1
      ) {
        throw new HttpError(
          400,
          response.url,
          errorsHandler.parse(parsedResponse.errors),
        );
      } else {
