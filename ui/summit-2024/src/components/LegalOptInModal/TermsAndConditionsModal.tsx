...
const TermsAndConditionsModal = () => {
    ...
    const renderContent = () => {
        ...
        const renderListItems = (list: List[]) => list.map((item, index) => (
            <Box key={index} sx={{ mt: 1 }}>
                <Typography sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px' }} dangerouslySetInnerHTML={{ __html: `${item.number} ${item.content.join(" ")}` }} />
            </Box>
        ));
        ...
        switch (currentTab) {
            case 0: {
                return <>
                    <Typography sx={{ color: 'var(--neutralLightest, #FAF9F6)', fontFamily: 'Dosis', fontSize: '32px', fontWeight: 700, lineHeight: '36px' }}>
                        {termsData.title}
                    </Typography>
                    <Typography sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px', mt: 2 }}>
                        {termsData.date}
                    </Typography>
                    {termsData.sections.map((section, index) => (
                        <Box key={index} sx={{ mt: 4 }}>
                            <Typography sx={{ color: 'var(--neutralLightest, #FAF9F6)', fontFamily: 'Dosis', fontSize: '24px', fontWeight: 700, lineHeight: '28px' }}>
                                {section.title}
                            </Typography>
                            {section.content?.map((paragraph, pIndex) => (
                                <Typography key={pIndex} sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px', mt: 1 }} dangerouslySetInnerHTML={{ __html: paragraph }} />
                            ))}
                            {section.subsections?.map((subsection, sIndex) => (
                                <Box key={sIndex} sx={{ mt: 2 }}>
                                    ...
                                    {subsection.content.map((content, cIndex) => (
                                        <Typography key={cIndex} sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px', mt: 1 }} dangerouslySetInnerHTML={{ __html: content }} />
                                    ))}
                                    {subsection.definitions && Object.entries(subsection.definitions).map(([key, value], dIndex) => (
                                        <Typography key={dIndex} sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px', mt: 1 }} dangerouslySetInnerHTML={{ __html: value }} />
                                    ))}
                                </Box>
                            ))}
                        </Box>
                    ...
                    {termsData.disclaimer && (
                        <Box sx={{ mt: 4 }}>
                            ...
                            {termsData.disclaimer.content.map((content, index) => (
                                <Typography key={index} sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px', mt: 1 }} dangerouslySetInnerHTML={{ __html: content }} />
                            ))}
                        </Box>
                    )}
                    ...
                    {termsData.contactus && (
                        <Box sx={{ mt: 4 }}>
                            ...
                            <Typography sx={{ color: 'var(--neutralLight, #D2D2D9)', fontSize: '16px', fontWeight: 500, lineHeight: '24px', mt: 1 }} dangerouslySetInnerHTML={{ __html: termsData.contactus.content }} />
                        </Box>
                    )}
    ...
    const checkedIcon = (
        <Box sx={{
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M7.875 15.167L2.917 10.208 4.125 9.021 7.875 12.771 15.854 4.792 17.063 5.979 7.875 15.167Z" fill="#121212"/>
            </svg>
        </Box>
    );
        ...
        >
            ...
            >
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    />
                    <Typography sx={{ color: theme.palette.text.neutralLightest, marginLeft: theme.spacing(1) }}>
                ...
                <Box sx={{ display: 'flex', gap: theme.spacing(2) }}>
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            alignItems: 'center',
                            borderRadius: '12px',
                            background: isChecked ? 'linear-gradient(70deg, #0C7BC5 -105.24%, #40407D -53.72%, #EE9766 -0.86%, #EE9766 103.82%)' : 'var(--neutral, #737380)',
                            color: 'white',
                            textTransform: 'none'
                        }}
                    >
