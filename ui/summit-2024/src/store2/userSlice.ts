...
export const userSlice = createSlice({
  ...
  reducers: {
    setConnectedPeerWallet: (
      state: StateProps,
      action: PayloadAction<{ peerWallet: boolean }>,
    ) => {
    ...
    setIsReceiptFetched: (
      state: StateProps,
      action: PayloadAction<{ isFetched: boolean }>,
    ) => {
      state.isReceiptFetched = action.payload.isFetched;
    },
    setSelectedProposal: (
      state: StateProps,
      action: PayloadAction<{ proposal: VoteReceipt["proposal"] }>,
    ) => {
      state.proposal = action.payload.proposal;
    },
    setUserVotes: (
      state: StateProps,
      action: PayloadAction<{ userVotes: UserVotes[] }>,
    ) => {
      state.userVotes = action.payload.userVotes;
    },
    setEventData: (
      state: StateProps,
      action: PayloadAction<{ event: EventPresentation }>,
    ) => {
    ...
    setWinners: (
      ...
      let filteredWinners = state.winners.filter(
        (oldWinner) =>
          !action.payload.winners.some(
            (winner) => winner.categoryId === oldWinner.categoryId,
          ),
      );
    ...
    setUserStartsVerification: (
      ...
      action: PayloadAction<{
        stakeAddress: string;
        verificationStarts: VerificationStarts;
      }>,
    ) => {
      state.userVerification[action.payload.stakeAddress] =
        action.payload.verificationStarts;
    },
  },
});
