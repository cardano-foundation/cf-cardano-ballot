...
export const formatUTCDate = (date: string) => {
  ...
  return `${isoDate.substring(0, 4)} ${monthName} ${isoDate.substring(
    8,
    10,
  )}th ${isoDate.substring(11, 16)} UTC`;
};
...
export const getDateAndMonth = (date: string) => {
  ...
  return `${+isoDate.substring(8, 10)} ${getMonthName(
    +isoDate.substring(5, 7) - 1,
  )}`;
};
